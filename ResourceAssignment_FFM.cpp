...
void ResourceAssignment::handle_requests (CircuitRequest * circuitRequest) {
	...
	unsigned int NumofGB = 1; // must be 1 here
	...
	if (circuitRequest->OccupiedSpectralSlots >= AvailableSpecSlots.size ()) {
		...
		for (i = AvailableSpecSlots.begin (); i != AvailableSpecSlots.end (); i++) {
			if (AssignedSS.empty ()) {
				...
				if ((circuitRequest->OccupiedSpectralSlots + NumofGB) == AssignedSS.size ()) {
					AvailableFlag = true;
					break;
				} 
				else {
	...
	if (AvailableFlag == false) {
		...
		network->TotalSSOccupied += (circuitRequest->OccupiedSpectralSlots + NumofGB) * mfTimes;
	}
